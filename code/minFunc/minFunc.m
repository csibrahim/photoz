function [x,f,exitflag,output,steps] = minFunc(funObj,x0,options,varargin)
...
% Output Log
if verboseI
    fprintf('%10s %10s %15s %15s %15s %15s %15s %15s\n','Iteration','FunEvals','Step Length','Function Val','Opt Cond','Training','Testing','Time');
end

if logfile
    fid = fopen(logfile, 'a');
    if (fid > 0)
        fprintf(fid, '-- %10s %10s %15s %15s %15s 15s 15s 15s\n','Iteration','FunEvals','Step Length','Function Val','Opt Cond','Training','Testing','Time');
        fclose(fid);
    end
end
...
for i = 1:maxIter
    ...
    if verboseI
        fprintf('%10d %10d %15.5e %15.5e %15.5e %15.5f %15.5f %15.2f\n',i,funEvals*funEvalMultiplier,t,f,sum(abs(g)),trainCost,validCost,time);
    end

    if logfile
        fid = fopen(logfile, 'a');
        if (fid > 0)
            fprintf(fid, '-- %10d %10d %15.5e %15.5e %15.5e %15.5f %15.5f %15.2f\n',i,funEvals*funEvalMultiplier,t,f,sum(abs(g)),trainCost,validCost,time);
            fclose(fid);
        end
    end
