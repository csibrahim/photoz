...
\begin{figure*}
       ...
       ~
        ...
        \caption{Density scatter plots of the true $z_\textrm{s}$ versus the predicted $z_\textrm{p}$ after training the GP-VC model with samples with $RIZ<23$ (top) and $RIZ<22$ (bottom) using $m=10$ basis functions with (a) zero mean, (b) linear regression, (c) joint linear and non-linear optimization and (d) {\sc ANNz}. The plots shows the performance on the same test set, the colours however are scaled differently according to the density range in each plot to avoid colour saturation}
...
\begin{figure}
       ...
       \caption{The density scatter plot for (a) the final {\sc ANNz} model using a committee of 5 networks with 5:20:20:1 architectures based on the best performing model from Figure \ref{fig-ann-gpvc-sdss} and (b) the final GP-VC model trained using $m=103$ basis functions with a jointly optimized linear mean function on SDSS DR12. }
       \label{fig-final-model-sdss}
\end{figure}

 \begin{table*}
\caption{The density scatter plot for (a) the final {\sc ANNz} model using a committee of 5 networks with 5:20:20:1 architectures based on the best performing model from Figure \ref{fig-ann-gpvc-sdss} and (b) the final GP-VC model trained using $m=103$ basis functions with a jointly optimized linear mean function on the the SDSS DR12 dataset.}
\begin{center}
\begin{tabular}{| l | c | c |  c | c |  c | c |  c | c |  c | c | }
     				&	$\Delta z$	&	$\Delta z_\textrm{norm}$	&	max$_{z}$ & max$_{norm}$		&	$\mu_{z}$&	$\mu_{norm}$	& $\sigma_{z}$ & $\sigma_{norm}$ & out$_{z}$&out$_{norm}$\\	\hline
