...
It is further assumed that the noise $\epsilon$ is normally distributed with a mean of zero and variance $\sigma_{n}^{2}$, or $\epsilon\sim\mathcal{N} \left(0,\sigma_{n}^{2}\right)$. To simplify the notation, it is assumed that $y\sim\mathcal{N} \left(0,1\right)$ (this can readily be achieved without loss of generality) and univariate, although the derivation can be easily extended to multivariable problems. Therefore, the conditional probability of the observed variable given the function is distributed as follows:
\begin{equation}
p\left(\mathbf{y}|\mathbf{f}\right)\sim\mathcal{N} \left(\mathbf{f},\sigma_{n}^{2}\right).
\end{equation}

A GP then proceeds by applying a \emph{Bayesian} treatment to the problem, namely by using Bayes' rule and the rules of probability to fit a probability distribution over the space of possible functions $\mathbf{f}$ given the data:
...
This requires us to assume a prior, $p\left(\mathbf{f}|\mathbf{X}\right)$, over our space of functions. A reasonable prior, is to assume that the function is normally distributed with a mean of zero, to match the mean of the normalized variable $\mathbf{y}$, and a covariance \emph{function} $\mathbf{K}$, or $p\left(\mathbf{f}|\mathbf{X}\right)\sim\mathcal{N} \left(0,\mathbf{K}\right)$. The covariance function has to capture some of our prior knowledge about the relationships between the observables. A sensible assumption, is that inputs that are similar in the input space, should also be similar in the output space. The definition of similarity is a modelling decision that can differ across different domains and applications, but generally, $\mathbf{K}$ has to reflect that by assigning higher values to more similar pairs. $\mathbf{K}$ therefore can be modelled as a function of the input $\mathbf{X}$, $\mathbf{K}=k\left(\mathbf{X},\mathbf{X}\right)$, where each element $\mathbf{K}_{ij}=k\left(\mathbf{x}_{i},\mathbf{x}_{j}\right)$ and $k$ is some function of similarity between two inputs. For $\mathbf{K}$ to be a proper covariance, it has to be symmetric and positive-definite matrix, not any arbitrary function $k$ can guarantee that. A class of functions that guarantees the structural constraints of the covariance matrix are called \emph{kernels} \citep{mercer1909}. A commonly used kernel function, which is the focus of this work, is the squared exponential kernel, defined as follows:

\begin{equation}
k\left(\mathbf{x}_{i},\mathbf{x}_{j}\right) = \sigma^{2}\exp\left(-\frac{1}{2\lambda^{2}}\left\|\mathbf{x}_{i}-\mathbf{x}_{j}\right\|^{2}\right),
\end{equation}

where $\sigma$ and $\lambda$ are referred to as the height (output, or variance) and characteristic length (input) scale respectively, which we will assume fixed for now. The similarity between two input vectors, under the squared exponential kernel, is a non-linear function of the Euclidean distance between them, which is a reasonable assumption in most applications. Now that we have our likelihood $p\left(\mathbf{y}|\mathbf{f}\right)$ and prior $p\left(\mathbf{f}|\mathbf{X}\right)$, the marginal likelihood $p\left(\mathbf{y}|\mathbf{X}\right)$ can be computed as follows:
...
If we assume that the data are independent and identically distributed (iid), the marginal likelihood can be computed as follows:
\begin{equation}
...
The main draw back of GPs is the $O\left(n^{3}\right)$ computational cost required to invert the $n\times n$ matrix $\mathbf{K_{xx}}+\sigma_{n}^{2}\mathbf{I}$. Here where the sparse Gaussian Process approximation come in to reduce the computation cost of GP inference, which will be discussed in the following section.
