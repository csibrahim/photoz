...
\begin{enumerate}
  ...
  \item The proposed approach was applied to a simulated catalogue and achieved  a mean $\Delta z = 0.004(1+z_{spec})$ exceeding the requirement for the Euclid mission.
\end{enumerate}
...
\begin{center}
  \begin{tabular}{| l | c | c | c | c | }
	...
	Zero						&	0.0765		&	0.0301				&	0.1404		&	0.0531					\\
	Linear Regression			&	0.0387		&	0.0186				&	0.1823		&	0.0701					\\
	Joint						&	\textbf{0.0354}	&	\textbf{0.0160}			&	\textbf{0.0535}	&	\textbf{0.0245}				\\	\hline
  \end{tabular}
\end{center}
...
(\textbf{SR: Is there anything interesting to say here from the Astro perspective? Do these 'filters' into the data space have any 'meaning'?})
...
We note that although the \emph{training} complexity costs require effort for large numbers of basis functions, once all parameters are inferred we enjoy effectively a linear basis model performance running over unseen (test) data. We therefore consider the performance for a realistic, yet large, number of functions. The final GP-VC model to target the Euclid Space Mission was trained with $m=200$ basis functions normalised, balanced and with a joint mean optimisation reached a mean $\Delta z_{norm}=0.004$ and a $max\left(\Delta z_{norm}\right)=0.0416$ on the test set. The density scatter plot for the final model is shown in Figure \ref{fig-final-model}.
