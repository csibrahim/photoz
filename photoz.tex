...
where $\sigma$ and $\lambda$ are referred to as the hight-variance and the length-scale respectively, which we will assume fixed for now. The similarity between two input vectors, under the squared exponential kernel, is a non-linear function of the Euclidean distance between them, which is a reasonable assumption in most applications. Now that we have our likelihood $p\left(\mathbf{y}|\mathbf{f}\right)$ and prior $p\left(\mathbf{f}|\mathbf{X}\right)$, the marginal likelihood $p\left(\mathbf{y}|\mathbf{X}\right)$ can be computed as follows:
